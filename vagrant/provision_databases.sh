...
# MySQL
export MYSQL_PWD="W0mb4t666!"

# Restore Django MySQL database
SHOW=`echo "show databases" | mysql -uroot | grep -P '^plugins_django$'`
if [ "$SHOW" != "plugins_django" ]; then
    echo "MySQL Django database does not exist, creating ..."
    echo "create database plugins_django" | mysql -uroot
    echo "Done."
    echo "Restoring Django database ..."
    mysql -uroot plugins_django < /vagrant/vagrant/django.mysql
    echo "Done."
else
...
SHOW=`echo "show databases" | mysql -uroot | grep -P '^plugins_moodle$'`
if [ "$SHOW" != "plugins_moodle" ]; then
    echo "MySQL Moodle database does not exist, creating ..."
    echo "create database plugins_moodle" | mysql -uroot
    echo "Done."
    echo "Restoring Moodle database ..."
    mysql -uroot plugins_moodle < /vagrant/vagrant/moodle.mysql
    echo "Done."
else
